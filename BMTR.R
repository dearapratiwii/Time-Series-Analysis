...
data = read.csv("D:/Semester 6/Analisis Deret Waktu/EAS/BMTR.JK.csv")
data
data[data$Volume==0,]
data[data$Close=="null",]
data1 = data[-c(23, 45, 64, 74, 76, 84, 105, 107, 108, 109, 110, 111, 307, 308),]
data1

# Deklarasi Variabel ihsg dan date
bmtr = as.numeric(as.character(data1$Close))
date = as.Date(data1$Date, origin = "2019-01-04")
plot(date,ihsg,type="l")
write.table(ihsg, "D:/ihsg.txt")

# Time Series Plot
library(ggplot2)
tmp <- data.frame(BMTR=bmtr,
                  Tanggal=date)
p <- ggplot(tmp, aes(Tanggal, BMTR)) +
  geom_line(colour = "blue") +
  geom_vline(xintercept= tmp$Tanggal[286],
